...
const CompletedPollinationDashboard = () => {
  ...
  useEffect(() => {
    const fetchPollinationData = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API}/api/v1/Dashboard/completed/week/${user.userId}`, {
  ...
  const processData = () => {
    ...
    pollinationData.forEach((item) => {
      const { gourdType, week, year, plotNo, totalCompleted } = item;
...
export default CompletedPollinationDashboard;